...
const normalizeNumericString = (value: string | number | null | undefined) => {
    ...
    if (lastDotIndex > -1) {
        return dotCount > 1 ? textValue.replace(/\./g, '') : textValue;
    }
...
const initialProjectionYears =
    parseNumber(props.defaultAssumptions.projection_years || 5) || 5;
const initialGrowthRate =
    parseNumber(props.defaultAssumptions.roe || 24) *
    (1 - parseNumber(props.defaultAssumptions.payout || 75) / 100);
...
const form = useForm({
    ...
    current_price_per_share:
        props.defaultAssumptions.current_price_per_share ??
        props.investiment.value.toString(),
    growth_rates: props.defaultAssumptions.growth_rates?.length
        ? [...props.defaultAssumptions.growth_rates]
        : Array.from({ length: initialProjectionYears }, () =>
              initialGrowthRate.toFixed(2),
          ),
});
...
const currentPrice = computed(() =>
    parseNumber(form.current_price_per_share || props.investiment.value),
);
    ...
    return [
        {
            ...
            value:
                props.valuation.summary.market_cap !== null
        ...
        },
        {
            ...
            value: formatCurrency(
                props.valuation.summary.present_value_of_cash_flows,
            ),
            tone: 'emerald',
        },
        {
            ...
            value: formatCurrency(
                props.valuation.summary.terminal_present_value,
            ),
            tone: 'cyan',
        },
        {
...
const submit = () => {
    form.transform((data) => ({
        ...
        current_price_per_share: normalizeNumericInput(
            data.current_price_per_share,
        ),
        growth_rates: data.growth_rates.map((growthRate) =>
            normalizeNumericInput(growthRate),
        ),
    }));

    if (props.editingValuationId) {
        form.put(`/valuations/${props.editingValuationId}`);
        return;
    }
                ...
                <div class="flex items-start justify-between gap-4">
                    <div class="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            >
                                Preencha as premissas principais e veja o
                                resultado consolidado ao lado.
                            </p>
                        </div>
                    </div>
                    <button
        ...
        >
            <div class="space-y-6">
                <section class="rounded-xl border border-border bg-card p-5">
                    <div class="mb-5 flex items-start justify-between gap-4">
                        ...
                        >
                            {{
                                investiment.type_label ||
                                investiment.portfolio_class ||
                                'Não classificado'
                            }}
                        </div>
                    </div>
                    ...
                    <div class="grid gap-3 md:grid-cols-2">
                        ...
                        >
                            <label
                                class="mb-2 block text-sm font-medium text-muted-foreground"
                                >Market cap estimado</label
                            >
                            <div
                                class="rounded-lg border border-investment/40 bg-investment/10 px-3 py-2 font-semibold text-investment"
                            >
                    ...
                    <div class="grid gap-3 md:grid-cols-2">
                        ...
                        >
                            <Label
                                required
                                class="mb-2 block text-sm font-medium text-muted-foreground"
                                >Lucro líquido atual</Label
                            >
                            <CurrencyInput
                                v-model="form.current_fcf"
                                :error="form.errors.current_fcf"
                                placeholder="0,00"
                            />
                        </div>
                        ...
                        >
                            <label
                                class="mb-2 block text-sm font-medium text-muted-foreground"
                                >Crescimento base estimado</label
                            >
                            <div
                                class="rounded-lg border border-investment/40 bg-investment/10 px-3 py-2 font-semibold text-investment"
                            >
                ...
                <section class="rounded-xl border border-border bg-card p-5">
                    <div class="mb-5 flex items-start justify-between gap-4">
                        ...
                        <div
                            class="rounded-full border border-border bg-surface px-3 py-1 text-xs text-muted-foreground"
                        >
                            {{ projectionYearIndexes.length }} ano(s) a partir
                            de {{ currentCalendarYear }}
                        </div>
                    </div>
                    ...
                    >
                        ...
                        >
                            ...
                            <tbody class="divide-y divide-border bg-surface">
                                <tr
                                    v-for="forecast in yearlyForecasts"
                                    :key="forecast.yearIndex"
                                    class="hover:bg-surface/80"
                                >
                                    <td
                                        class="px-4 py-4 font-medium text-muted-foreground"
                                    >
                                        {{ forecast.calendarYear }}
                                    </td>
                                    <td class="px-4 py-4">
                                        <div
                                            class="rounded-lg border border-revenue/40 bg-revenue/15 px-3 py-2"
                                    ...
                                    <td
                                        class="px-4 py-4 text-right font-semibold text-primary"
                                    >
                                        {{
                                            forecast.forecast
                                                ? formatCurrency(
            ...
            <aside class="space-y-6">
                <section class="rounded-xl border border-border bg-card p-5">
                    ...
                    <div
                        v-else
                        class="rounded-lg border border-dashed border-border bg-surface p-4 text-sm text-muted-foreground"
                    >
            ...
            <div
                class="flex flex-wrap items-center justify-end gap-3 pt-2 xl:col-span-2"
            >
