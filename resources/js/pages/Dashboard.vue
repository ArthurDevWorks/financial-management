...
import { DollarSign, TrendingDown, TrendingUp } from 'lucide-vue-next';
...
const summaryCards = [
    ...
    },
    {
        ...
        variant: (props.summary.netBalance >= 0 ? 'profit' : 'expense') as
            | 'profit'
            | 'expense',
        icon: TrendingUp,
    },
    ...
    <AppLayout>
        <div class="p-8">
            <PageHeader
                ...
                description="Visualize um resumo completo de suas finanças"
            >
                <template #actions>
            ...
            <!-- SUMMARY CARDS -->
            <div
                class="mb-8 grid grid-cols-1 gap-5 md:grid-cols-2 lg:grid-cols-4"
            >
                ...
                <template v-if="loading">
                    <SectionCard title="Receitas por Categoria">
                        <div class="flex items-center justify-center py-8">
                            <Skeleton
                                class="h-[260px] w-[260px] rounded-full"
                            />
                        </div>
                    </SectionCard>
                ...
                <template v-else>
                    <div class="animate-fade-in-up animate-stagger-5">
                        <SectionCard
                            ...
                            :description="`${topRevenueCategories.length} categoria(s)`"
                        >
                            <DoughnutChart :data="topRevenueCategories" />
                            <div
                                v-if="!topRevenueCategories.length"
                    ...
                    <div class="animate-fade-in-up animate-stagger-6">
                        <SectionCard
                            ...
                            :description="`${topExpenseCategories.length} categoria(s)`"
                        >
                            <DoughnutChart :data="topExpenseCategories" />
                            <div
                                v-if="!topExpenseCategories.length"
                                class="py-8 text-center text-sm text-muted-foreground"
                            >
                    ...
                    <template v-else>
                        <template v-if="accountsEvolution.length">
                            <p class="mb-4 text-sm text-muted-foreground">
                                {{ accountsEvolution.length }} conta(s)
                            </p>
                            <Table>
                                <TableHeader>
                                    <TableRow
                ...
                <SectionCard title="Transações Recentes">
                    ...
                    <template v-else>
                        <template v-if="recentTransactions.length">
                            <p class="mb-4 text-sm text-muted-foreground">
                                {{ recentTransactions.length }} transação(ões)
                            </p>
                            <Table>
                                <TableHeader>
                                    <TableRow
