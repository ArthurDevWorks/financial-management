...
type SummaryVariant = 'default' | 'revenue' | 'expense' | 'investment' | 'profit';
...
const variantStyles: Record<SummaryVariant, { border: string; icon: string; value: string; trend: string }> = {
  default: {
...
<template>
  ...
  >
    ...
    <p class="text-[1.75rem] font-bold leading-tight tracking-tight" :class="variantStyles[variant].value">
      {{ value }}
    </p>
    ...
    <div v-if="trend !== undefined" class="mt-2 flex items-center gap-2">
      <span
        ...
        :class="[
          trend >= 0 ? variantStyles[variant].trend : 'bg-destructive/10 text-destructive',
        ]"
      >
      ...
      <span v-if="trendLabel" class="text-xs text-muted-foreground">{{ trendLabel }}</span>
    </div>
  </div>
</template>
