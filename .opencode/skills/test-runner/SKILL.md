...
Read `PROJECT_CONTEXT.md` → `## 2. Stack de Tecnologia — Comandos de Dev` and verify:
...
Use the **Comando de Teste** from `PROJECT_CONTEXT.md` (backend):
```bash
...
Use the backend **Comando de Teste** with integration tag/directory if defined in PROJECT_CONTEXT.md.
...
### Test Isolation
Before running tests, use **Reset do Banco de Dados de Teste** from `PROJECT_CONTEXT.md` if applicable.
