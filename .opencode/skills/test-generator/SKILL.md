...
- **TDD-First (executor (TDD mode)):** When writing tests BEFORE implementation — tests must be designed to FAIL initially (red phase). Use mocks/stubs/interfaces for dependencies that don't exist yet.
...
### TDD-First Mode (for executor (TDD mode))
