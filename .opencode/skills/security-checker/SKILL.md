...
### Prerequisites
Read `PROJECT_CONTEXT.md` → `## 2. Stack de Tecnologia — Comandos de Dev` and run:
...
Run commands from `PROJECT_CONTEXT.md → Comandos de Dev`:
```bash
...
- Uses: **Security Scanner** commands from `PROJECT_CONTEXT.md → Comandos de Dev` via terminal
